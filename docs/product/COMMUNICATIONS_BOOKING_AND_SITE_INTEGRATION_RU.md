...
4. Чат сайта клиники:
   - это собственный помощник TemichevVET без внешнего мессенджера;
   - простые подтверждённые вопросы о режиме работы, адресе, услугах, ценах, стационаре и записи обрабатываются на сайте;
   - бот не оценивает состояние животного и не ставит диагноз;
   - вопрос сотруднику отправляется в отдельную базу публичного owner-gateway;
   - CRM сама забирает сообщение исходящим запросом и сохраняет его как `OnlineAppointmentRequest` с источником `CLINIC_SITE_CHAT`;
   - сообщение не создаёт приём автоматически и не открывает публичный доступ к клинической базе.
5. Аналитика сайта клиники:
   - Яндекс.Метрика включается только по счётчику, принадлежащему клинике, и после согласия посетителя;
   - собственные обезличенные события сохраняются в отдельной базе owner-gateway;
   - имя, телефон, текст чата, медицинская информация, полный URL и IP-адрес в аналитике не сохраняются;
   - директор видит в закрытой CRM посещения, источники, разделы и действия записи/звонка/чата/маршрута;
   - публичный сайт не получает доступ к отчётам и клинической базе.
...
## Контракт сайта клиники

Сайт обращается только к публичному owner-gateway. Клиническая CRM остаётся недоступной из интернета.
...
Для собственного чата используется `POST /v1/public/clinic/inquiries`: имя, телефон, кличка питомца, текст сообщения, согласие, `clientRequestId` и скрытое honeypot-поле. Повтор с тем же `clientRequestId` не создаёт дубль. Ответ публичного API не содержит внутренних данных CRM.

Маршрут реализован в локальном коде, но не включён в production: до публикации нужны HTTPS, reverse proxy, разрешённый origin, миграция отдельной базы шлюза и отдельное разрешение на деплой.
...
10. Помощник отвечает на организационный вопрос без внешнего сервиса.
11. Вопрос сотруднику один раз появляется в CRM даже после повторного опроса шлюза.
12. Срочное или медицинское обращение не получает автоматический диагноз и предлагает позвонить в клинику.
13. Без согласия посетителя Яндекс.Метрика и собственный счётчик не отправляют события.
14. Поля формы и текст чата отсутствуют в событиях аналитики.
15. Сводка сайта открывается только директору через защищённый внутренний маршрут.
...
- HTTPS и reverse proxy для `POST /v1/public/clinic/inquiries`;
- `OWNER_GATEWAY_PUBLIC_SITE_ORIGINS` и проверка ограничения частоты по реальному IP;
- миграция `20260822000100_public_clinic_inquiries` только в отдельной базе owner-gateway;
- миграция `20260827000100_public_clinic_analytics` только в отдельной базе owner-gateway;
- принадлежащий клинике `VITE_YANDEX_METRIKA_ID` и цели из списка в документации сайта;
