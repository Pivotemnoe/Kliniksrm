...
import { Alert, Button, DatePicker, Form, Input, Modal, Radio, Select, Space, Typography } from 'antd';
...
const appointmentSchema = z
  .object({
    ...
    endsAt: z
      ...
      .pipe(z.string().refine((value) => Boolean(fromDateTimeText(value)), 'Например: 25.06.2026 10:45').optional()),
...
export function AppointmentFormDrawer({
  ...
  function submit(values: AppointmentFormValues) {
    ...
    onSubmit({ appointment, animalEdit: animalEdit?.animalId === values.animalId ? animalEdit : undefined });
  }
  ...
  return (
    ...
    >
      <Form layout="vertical">
        ...
        <RegistrationLastVisit ownerId={ownerId} isNew={clientMode === 'new'} open={open} />
        {clientMode === 'new' ? <Typography.Paragraph><Typography.Text strong>Последний приём пациента: </Typography.Text>Приёмов ещё нет</Typography.Paragraph> : null}
        {canCreateCards ? (
          <Controller
            control={control}
            name="clientMode"
        ...
        {clientMode === 'existing' ? (
          ...
          <div className="form-grid two-columns">
            ...
            <Controller control={control} name="birthDate" render={({ field, fieldState }) => (
              <Form.Item label="Дата / год рождения" validateStatus={fieldState.error ? 'error' : undefined} help={fieldState.error?.message}>
                <Input {...field} placeholder="ГГГГ, ММ.ГГГГ или ДД.ММ.ГГГГ" />
              </Form.Item>
            )} />
            <Controller
              control={control}
