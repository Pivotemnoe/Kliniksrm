...
export class OrganizationController {
  ...
  @Get('print-profile')
  @RequireAnyPermissions('settings.read', 'documents.print')
  @ApiOkResponse({ description: 'Clinic identity fields allowed in printable forms.' })
  getPrintProfile() {
    return this.organizationService.getOrganizationPrintProfile();
  }

  @Get('print-logo')
  @RequireAnyPermissions('settings.read', 'documents.print')
  @Header('Cache-Control', 'private, no-store')
  @ApiOkResponse({ description: 'Current organization logo for printable forms.' })
  async getPrintLogo(@Res({ passthrough: true }) response: Response) {
