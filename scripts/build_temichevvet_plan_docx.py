...
def build():
    ...
    add_callout(doc, "Сейчас", f"{items[plan['current_focus']]['id']} — {items[plan['current_focus']]['title']}. Параллельно проверяем {items[plan['parallel_validation']]['id']} — {items[plan['parallel_validation']]['title']}.")
    ...
    sequence = [
        ...
        "Параллельно завершить матрицу подключения нового владельца на Android/TECNO и обработку заявки сотрудником.",
